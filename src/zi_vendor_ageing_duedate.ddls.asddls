...
@EndUserText.label: 'Vendor Ageing Due date'
@Metadata.ignorePropagatedAnnotations: true
@Metadata.allowExtensions: true
define view entity ZI_VENDOR_AGEING_DUEDATE
  with parameters
    P_Date : datum
  ...
  //  composition of target_data_source_name as _association_name
{
  ...
  key _Opr.NetDueDate,
      ...
      dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) as AgedDays,
      _Opr.CompanyCodeCurrency,
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
      when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 0 and 30
      then
      //      _Opr.AmountInCompanyCodeCurrency +  _Received.received
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 31 and 45
          then
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          _CustAgeing.BalanceAmount
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 46 and 60
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 61 and 90
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 91 and 120
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 121 and 180
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) between 181 and 360
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
      ...
      @Semantics: { amount : {currencyCode: 'CompanyCodeCurrency'} }
      case
          when dats_days_between( _Opr.NetDueDate, $parameters.P_Date ) > 360
          then
          _CustAgeing.BalanceAmount
      //          _Opr.AmountInCompanyCodeCurrency +  _Received.received
          else cast( 0 as abap.curr(15,2) )
